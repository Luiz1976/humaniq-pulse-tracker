...
-- Migration: CLEAN VERSION (Safe to re-run)
-- ============================================================================

-- Drop existing objects if they exist (safe to re-run)
DROP TABLE IF EXISTS public.website_content_activity_logs CASCADE;
DROP TABLE IF EXISTS public.website_content_versions CASCADE;
DROP TABLE IF EXISTS public.website_content_images CASCADE;
DROP TABLE IF EXISTS public.website_content_posts CASCADE;
DROP TABLE IF EXISTS public.website_content_schedule CASCADE;

-- Drop functions
DROP FUNCTION IF EXISTS public.update_updated_at_column() CASCADE;
DROP FUNCTION IF EXISTS public.create_content_version() CASCADE;
DROP FUNCTION IF EXISTS public.calculate_next_schedule(TEXT, TIMESTAMP WITH TIME ZONE) CASCADE;
DROP FUNCTION IF EXISTS public.generate_slug(TEXT, TEXT) CASCADE;
...
BEGIN
  IF (OLD.title IS DISTINCT FROM NEW.title) OR (OLD.content IS DISTINCT FROM NEW.content) THEN
    INSERT INTO public.website_content_versions (
      ...
      meta_title, meta_description, keywords, changed_by
    ) VALUES (
      OLD.id, OLD.version, OLD.title, OLD.content, OLD.excerpt,
      OLD.meta_title, OLD.meta_description, OLD.keywords, current_user
    );
...
CREATE FUNCTION public.calculate_next_schedule(
  p_frequency TEXT,
...
CREATE FUNCTION public.generate_slug(p_title TEXT, p_route TEXT)
RETURNS TEXT AS $$
DECLARE
  v_slug TEXT;
  v_counter INTEGER := 0;
...
CREATE POLICY "Service role full access posts" ON public.website_content_posts FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Service role full access schedule" ON public.website_content_schedule FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Service role full access images" ON public.website_content_images FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Service role full access versions" ON public.website_content_versions FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Service role full access logs" ON public.website_content_activity_logs FOR ALL TO service_role USING (true) WITH CHECK (true);

CREATE POLICY "Authenticated view published" ON public.website_content_posts FOR SELECT TO authenticated USING (status = 'published');
CREATE POLICY "Authenticated view schedule" ON public.website_content_schedule FOR SELECT TO authenticated USING (true);
CREATE POLICY "Authenticated view images" ON public.website_content_images FOR SELECT TO authenticated USING (true);
CREATE POLICY "Authenticated view logs" ON public.website_content_activity_logs FOR SELECT TO authenticated USING (true);

CREATE POLICY "Public view published" ON public.website_content_posts FOR SELECT TO anon USING (status = 'published');
CREATE POLICY "Public view images" ON public.website_content_images FOR SELECT TO anon USING (true);
...
COMMENT ON TABLE public.website_content_posts IS 'Generated content for HumaniQ AI website';
COMMENT ON TABLE public.website_content_schedule IS 'Automatic content generation schedule';
COMMENT ON TABLE public.website_content_images IS 'Images associated with posts';
COMMENT ON TABLE public.website_content_versions IS 'Content version history';
COMMENT ON TABLE public.website_content_activity_logs IS 'Activity logs for monitoring';
...
AND table_name LIKE 'website_content%';
